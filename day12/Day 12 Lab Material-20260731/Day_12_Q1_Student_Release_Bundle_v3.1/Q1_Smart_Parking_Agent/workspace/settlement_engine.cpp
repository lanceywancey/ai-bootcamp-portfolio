...
void validate(const Input& i) {
    ...
    if (i.soc_after_export < 0 || i.soc_after_export > 100 || i.minimum_departure_soc < 0 || i.minimum_departure_soc > 100) throw std::invalid_argument("invalid state of charge");
    ...
    if (i.export_wh > 0 && i.soc_after_export < i.minimum_departure_soc) throw std::invalid_argument("minimum departure SOC violated");
}
...
int calculate_energy_amount(int wh, int rate) { return static_cast<int>((static_cast<long long>(wh) * rate + 500) / 1000); }
Result calculate_settlement(const Input& i) { int p=calculate_parking_fee(i), in=calculate_energy_amount(i.import_wh,i.import_rate_cents_per_kwh), out=calculate_energy_amount(i.export_wh,i.export_rate_cents_per_kwh); return {p,in,out,p+in-out}; }
